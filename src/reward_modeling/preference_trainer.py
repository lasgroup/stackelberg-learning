...
from transformers import (
    PreTrainedModel, DataCollator, PreTrainedTokenizerBase, BaseImageProcessor,
    FeatureExtractionMixin, ProcessorMixin, EvalPrediction, TrainerCallback,
    Trainer, is_wandb_available
)
from transformers.utils import is_peft_available
from transformers.trainer_pt_utils import nested_detach
from trl import RewardConfig, maybe_apply_chat_template
...
from src.reward_modeling.data_collator_with_padding import PreferenceDataCollatorWithPadding
...
def _tokenize(batch: dict[str, list[Any]], tokenizer: "PreTrainedTokenizerBase") -> dict[str, list[Any]]:
    ...
    new_examples = {
        "input_ids_messages": [],
        "attention_mask_messages": [],
    }
    for messages in batch["text"]:
        tokenized_messages = tokenizer(messages)
        new_examples["input_ids_messages"].append(tokenized_messages["input_ids"])
        new_examples["attention_mask_messages"].append(tokenized_messages["attention_mask"])
    return new_examples
...
class PreferenceTrainer(Trainer):
    ...
    def __init__(
        ...
        if data_collator is None:
            ...
            data_collator = PreferenceDataCollatorWithPadding(processing_class)
        ...
        if "input_ids_messages" not in train_dataset.column_names:
            with PartialState().local_main_process_first():
                fn_kwargs = {"tokenizer": processing_class}
                train_dataset = train_dataset.map(maybe_apply_chat_template, fn_kwargs=fn_kwargs)
                ...
                train_dataset = train_dataset.filter(
                    lambda x: len(x["input_ids_messages"]) <= max_length,
                    num_proc=args.dataset_num_proc,
                )
    ...
    def compute_loss(
        ...
        logits = model(
            input_ids=inputs["input_ids_messages"],
            attention_mask=inputs["attention_mask_messages"],
            return_dict=True,
        )["logits"]
        if "margin" in inputs:
        ...
        loss = nn.functional.binary_cross_entropy_with_logits(logits, inputs["labels"].unsqueeze(-1).to(logits.device))  # Shape: (batch_size, 1)
    ...
    def visualize_samples(self, num_print_samples: int):
        ...
        for _, inputs in enumerate(eval_dataloader):
            ...
            text = decode_and_strip_padding(inputs["input_ids_messages"], self.processing_class)
